...
namespace Microsoft_Teams_Graph_RESTAPIs_Connect.Models
{
    ...
    public class HttpHelpers
    {
        ...
        public async Task<T> HttpPost<T>(string uri, object body, string accessToken = "", int retries = 0, int retryDelay = 30)
        {
            string bodyString;
            if (body == null)
                bodyString = null;
            ...
            if (!response.IsSuccessStatusCode)
            {
                if (retries > 0)
                {
                    ...
                    return await HttpPost<T>(uri, body, accessToken, retries: retries - 1, retryDelay: retryDelay);
                }
                else
                    throw new Exception(responseBody);
            }
            T result = JsonConvert.DeserializeObject<T>(responseBody, jsonSettings);
            return result;
        }
